<div id="modal_form_privilageType" class="modal fade" tabindex="-1" style="overflow-y: scroll;">
    <div class="modal-dialog" style="width:700px;">
        <div class="modal-content">
            <!-- modal title -->
            <div class="modal-header no-padding">
                <h4 class="modal-title" id="defaultModalLabel">Form Privilage Type</h4>
            </div>
...
<script>
    ...
    function modal_form_privilageType_show(privilage_type_id) {
        clearData();
        modal_form_privilageType_set_field_value(privilage_type_id);
        $("#modal_form_privilageType").modal({backdrop: 'static'});
        if(privilage_type_id != 0)
            getDetails(privilage_type_id);
    }


    function modal_form_privilageType_set_field_value(privilage_type_id) {
         $("#privilage_type_id").val(privilage_type_id);
    }

    function modal_form_privilageType_set_value(privilage_type_id) {
    ...
    function getDetails(privilageTypeId){
        $.ajax({
            async: true,
            type: "POST",
            url: "<?php echo base_url().'privilageType/getDetails'; ?>",
            data:{
                        privilage_type_id:privilageTypeId
                    },
            //dataType: "json",
    ...
    function crud(){
        ...
        $.ajax({
            async: true,
            type: "POST",
            url: "<?php echo base_url().'privilageType/createUpdate'; ?>",
            data:{
                       action:action,
